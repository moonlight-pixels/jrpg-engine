...
class CharacterAnimationSetSpec extends Specification {
    void 'validate() raises error if any animations are missing'() {
        when:
        new CharacterAnimationSet(Mock(CharacterAnimationSet.Key), 0, 0, 1).validate()
    ...
    void 'validate() passes if no animations are missing'() {
        ...
        when:
        CharacterAnimationSet animationSet = new CharacterAnimationSet(Mock(CharacterAnimationSet.Key), 0, 0, 1)
